...
EXCHANGE_DATASET_ID = "crypto_currency"
EXCHANGE_TABLE_ID = "exchange_all"
...
def transform_kafka_json(df, spark_schema):
    df_stream = df \
        .select(F.from_json(F.col("value").cast('STRING'),spark_schema)).alias("exchange") \
...
if __name__ == "__main__":
    ...
    # read streaming data
    df_consume_stream = read_from_kafka(consume_topic=EXCHANGE_TOPIC)
    print(df_consume_stream.printSchema())

    # parse data
    df_assets = transform_kafka_json(df_consume_stream, exchange_spark_schema)
    print(df_assets.printSchema())

    # write output to Bigquery
    write_bq = sink_bigquery(df_assets, EXCHANGE_DATASET_ID, EXCHANGE_TABLE_ID, GCS_BUCKET)
