...
from config import EXCHANGE_TOPIC, BOOTSTRAP_SERVERS
...
if __name__ == '__main__':
    config = {
        ...
        'value_deserializer': lambda x: loads(x.decode('utf-8'), object_hook=lambda d: Exchange(d)),
        'group_id': 'consumer.group.id.json-example.1',
    }

    json_consumer = JsonConsumer(props=config)
    json_consumer.consume_from_kafka(topics=[EXCHANGE_TOPIC])